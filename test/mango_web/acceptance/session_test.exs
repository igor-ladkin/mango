defmodule MangoWeb.Acceptance.SessionTest do
  ...
  test "successful login for valid credentials" do
    ...
    find_within_element(form, :name, "session[email]") |> fill_field("john@example.com")
    find_within_element(form, :name, "session[password]") |> fill_field("secret")
  ...
  test "shows error message for invalid credentials" do
    ...
    find_within_element(form, :name, "session[email]") |> fill_field("john@example.com")
    find_within_element(form, :name, "session[password]") |> fill_field("not a secret")
    find_within_element(form, :tag, "button") |> click()
